...
// Interface for AI Service to make it easily replaceable
...
export class GeminiAIService implements IAIService {
  ...
  constructor() {
    ...
    if (apiKey && apiKey !== 'YOUR_GEMINI_API_KEY_HERE') {
      ...
      console.warn(
        'WARNING: GEMINI_API_KEY is not configured. Running in Mock/Fallback AI Mode.'
      );
    }
  }
...
JSON output:`;
  ...
  private getMockCategorization(description: string, currentDate: string) {
...
Coach:`;
  ...
  private getMockCoachResponse(message: string, context: any): string {
    const cleanMsg = message.toLowerCase();
    const activeGoals = context.savingsGoals.map((g: any) => `${g.name} (${g.progressPercent.toFixed(0)}% reached)`).join(', ') || 'No active goals';
    ...
    if (cleanMsg.includes('spend') || cleanMsg.includes('expense') || cleanMsg.includes('where am i spending') || cleanMsg.includes('most') || cleanMsg.includes('category') || cleanMsg.includes('categories')) {
      const topCategories = (Object.entries(context.categorySpending) as [string, number][])
        .sort((a, b) => b[1] - a[1])
        .slice(0, 4)
    ...
    if (cleanMsg.includes('how much') && (cleanMsg.includes('save') || cleanMsg.includes('every day') || cleanMsg.includes('daily') || cleanMsg.includes('monthly') || cleanMsg.includes('every month'))) {
      ...
      const ratesList = context.savingsGoals.map((g: any) => {
    ...
    if (cleanMsg.includes('save') || cleanMsg.includes('goal') || cleanMsg.includes('milestone') || cleanMsg.includes('progress')) {
      const goalList = context.savingsGoals.map((g: any) => `- **${g.name}**: Target ₹${g.targetAmount.toLocaleString()}, Current ₹${g.currentAmount.toLocaleString()} (${g.progressPercent.toFixed(0)}% complete) by ${g.targetDate}`).join('\n');
      if (!goalList) {
...
JSON output:`;
  ...
  private getMockReportInsights(stats: any) {
